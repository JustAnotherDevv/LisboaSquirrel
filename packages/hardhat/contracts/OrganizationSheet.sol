...
contract OrganizationSheet {
    ...
    function createOrganization(string memory _name, string memory _imgUrl, string memory _website, string memory _github, uint256 _tokenAmountEstimation) public {
...
=======
        ...
        organizationAmount++;
>>>>>>> 0bbb365 (Changed vote period end formula calc)

        emit OrganizationCreated(msg.sender, _name);
    }

    function editOrganization(uint256 organization, string memory _name, string memory _imgUrl, string memory _website, string memory _github, uint256 _tokenAmountEstimation) public onlySuperAdmin(organization) {
        require(organizations[organization].admins.length != 0, "No organization with this id.");
    ...
    function getCriteriaAtIndex(uint256 organization, uint256 index) public view returns (voteCriteria memory result) {
        require(index < organizations[organization].approvedCriteria.length, "Index out of bounds.");
        return organizations[organization].approvedCriteria[index];
    }

    function getCriteriaIdInOrganization(uint256 organization, string memory criteriaName) public view returns (uint256) {
    Organization storage org = organizations[organization];
    for (uint256 i = 0; i < uint256(org.approvedCriteria.length); i++) {
        if (keccak256(abi.encodePacked(org.approvedCriteria[uint256(i)].name)) == keccak256(abi.encodePacked(criteriaName))) {
            return i;
        }
    }
    ...
    function approveCriteria(uint256 organization, string memory _name, uint32 _range) public onlySuperAdmin(organization) {
        require(organizations[organization].admins.length != 0, "No organization with this id.");
    ...
    function removeCriteria(uint256 organization, string memory name) public onlySuperAdmin(organization) {
        require(organizations[organization].admins.length != 0, "No organization with this id.");
        Organization storage org = organizations[organizationAmount];
        uint256 criteriaId = getCriteriaIdInOrganization(organization, name);
        org.approvedCriteria[criteriaId] = org.approvedCriteria[org.users.length - 1];
    ...
    function getAllocationPeriodById(uint256 organization, uint256 index) public view onlySuperAdmin(organization) returns (votePeriod memory result) {
        require(index < organizations[organization].votingPeriods.length, "Index out of bounds.");
    ...
    function startAllocationPeriod(uint256 organization) public onlySuperAdmin(organization) {
        ...
        if (org.votePeriodAmount != 0) {
            require(!org.votingPeriods[org.votePeriodAmount - 1].isActive, "Can't add new voting period before finishing the previous one.");
        }
        org.votingPeriods.push(votePeriod(true, block.timestamp, 0));
        org.votePeriodAmount++;
    }

    function voteAllocationPeriod(uint256 organization, uint256 votePeriodIndex, address _votedOnUser, uint256 criteriaIndex, string memory note, uint32 rating ) public onlySuperAdmin(organization) {
        ...
        adminVotes[organization][votePeriodIndex][msg.sender][_votedOnUser][criteriaIndex] = voteCriteria({
        name: note,
        range: rating
    });
    }

    function endAllocationPeriod(uint256 organization, uint256 index ) public onlySuperAdmin(organization) {
        require(organizations[organization].admins.length != 0, "No organization with this id.");
        Organization storage org = organizations[organizationAmount];
        require(org.votingPeriods[index].isActive, "Allocation period has to be active.");
		...
		for (uint256 i = 0; i != getAdminAmount(organization); i++) {
			for (uint256 j = 0; j != getUserAmount(organization); j++) {
				for (uint256 k = 0; k != getCriteriaAmount(organization); k++) {
					adminVotes[organization][org.votingPeriods.length - 1][org.admins[i]][org.users[j]][k];
				}
			}
		}
